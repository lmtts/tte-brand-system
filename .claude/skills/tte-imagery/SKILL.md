...
name: tte-imagery
description: >
  Complete visual and photographic intelligence system for To The Ends of The Earth (TTE) — a Hope Channel sub-brand with Explorer archetype identity (National Geographic, Arc'teryx, Patagonia). Activate for ANY TTE imagery request: AI image prompt generation, photo sourcing, visual style explanation, image evaluation, art direction, shot lists, or keyword search. Triggers for: "create a TTE prompt", "does this image work for TTE?", "find photos for TTE", "what's the TTE visual style?", "I need a Scale/Stillness/POV/HUD image", or any vague scene description in TTE context. Contains all 4 narrative layer specs (Scale, Stillness, POV, HUD), modular AI prompt blocks, photographer references (McCurry, Burkard, Pellegrin, Salgado), color grades (Portra 400, Deakins, Lubezki, Bradford), and the full keyword library for Getty, Unsplash, Stocksy, Behance, Are.na. When in doubt, activate — the full system beats a generic answer every time.
---
...
2. Choose lighting, grade, and grain from the system
3. Produce a complete prompt + explain your creative choices
4. Offer alternatives for other layers
...
2. Pull from `references/keywords.md`
3. Include negative keywords to filter off-brand results
...
- `references/keywords.md` — Full keyword library for stock sourcing, platform search, negative keywords
