...
- ✅ **Fase 10 — 10 Tokens** *(2026-07-31)*: sem frame; proposta em código a partir de `tokens.json` e da seção 12/14 do `DESIGN.md`. **Última seção numerada — completa as 10.** Página única: esquerda explica a arquitetura em 3 camadas (primitives → brand → semantic, componentes só consomem semantic, nunca o valor bruto) + a regra "três faces, um sistema" (tokens ↔ dev ↔ Figma) + 4 botões de download (um por formato sincronizado: `.json`/`.css`/`.scss`/`.tailwind.js`). Direita: um painel estilo trace de código mostrando a cadeia real e completa de um token, do primitivo até o aplicado — `primitives.color.fire-orange (#FE5442)` → `brand.color.primary` → `semantic.color.surface.accent` ("CTA backgrounds, highlighted elements") — com um swatch Fire Orange no header do painel amarrando o abstrato ao visual. **Novo script de sync:** `scripts/sync-tokens-download.mjs` copia os 4 arquivos de `01-brand-system/tokens/` pra `public/downloads/` no `predev`/`prebuild`, mesmo padrão do kit de logos e do guia de voz — nunca duplicação manual.
- ✅ **Fase 11 — Polish global** *(iniciada 2026-07-31, concluída 2026-08-03 após uma segunda rodada de ajustes finos)*: escopo consolidado (planejamento original + notas do Lucas coletadas ao longo da montagem).
  - **A. Conteúdo & copy** ✅
    - [x] Revisão com o skill `humanizer` em todas as seções — copy já estava limpa (nascida sob as 11 regras editoriais do DESIGN.md), só achou **1 em-dash real**: alt text da Capa ("Snow peak... — the field..."), corrigido pra dois pontos. Comentários de código (`/** ... */`) não contam como conteúdo, ficaram intocados.
    - [x] Caça a viúvas tipográficas — script de medição via `Range.getClientRects()` (não estimativa visual) achou **5 viúvas reais** (The Brand, Color, Voice, System×2), todas corrigidas por reescrita (não CSS) até a última linha deixar de ser uma palavra solta. Falsos positivos descartados (listas `dt/dd` da Imagery, linha de 3+ palavras do Voice Terminology).
  - **B. Divisórias** ✅
    - [x] Linha horizontal no topo de cada `SectionShell`: peso cheio (`bg-paper/20`, full-bleed) marcando início de seção nova; peso leve e recuado (`bg-paper/8`, inset nas margens) entre páginas da mesma seção. Prop nova `dividerAbove="section"|"page"|"none"`.
    - [x] Linha vertical entre texto e conteúdo — só no modo `children`, omitida no modo `image` (full-bleed).
    - [x] *(ajuste 2026-08-03)* Posição e extensão corrigidas por pedido do Lucas: `left` de `calc(500*var(--u))` pra `calc(540*var(--u))` (mais pra direita, conforme screenshot de referência); `top`/`bottom` de valores fixos pra `0`/`0`, encostando nas divisórias horizontais do topo e do rodapé da seção.
  - **C. Navegação por seta** ✅
    - [x] `ArrowNav.tsx` novo: seta cima/baixo no teclado + botões fixos prev/next (canto inferior direito, acima do rodapé) — os dois juntos. Novo hook `useActivePageId` (granularidade de página, não só seção) pra saber exatamente onde parar. Navegação suprimida quando há um `[role="dialog"]` **modal aberto** (lightbox ou menu mobile), pra não navegar a página inteira por baixo dele.
    - [x] *(bug corrigido 2026-08-03)* Setas do teclado só davam um scroll curto em vez de trocar de seção. Causa: o guard de modal checava só `[role="dialog"]`, e o menu mobile (`MobileNav.tsx`) sempre tem esse atributo no DOM (mesmo fechado, só escondido por CSS) — então a navegação ficava permanentemente suprimida, o scroll nativo do navegador tomava conta, e o snap do Lenis revertia o resultado. Corrigido exigindo também `[aria-modal="true"]` no seletor.
  - **D. Motion de intel/HUD** ✅ *(construído 2026-08-03, referências do Lucas: fancycomponents.dev typewriter/scramble-hover/scramble-in + GSAP ScrambleText)*
    - [x] `Typewriter.tsx` novo — revela o heading de cada seção caractere a caractere com cursor piscando, disparado por `IntersectionObserver` (não no mount). Aplicado em todos os 11 headings de seção via `SectionShell`, substituindo o fade+slide antigo do heading (o body continua com fade próprio).
    - [x] `ScrambleHover.tsx` novo — texto embaralha em caracteres aleatórios e resolve da esquerda pra direita ao passar o mouse ou focar. Aplicado nos itens do índice de navegação (`SectionNav`, `MobileNav`) e nos botões de download (Logos, Voice, Typography, Tokens).
    - [x] `DecodeText` (já existente) estendido pra mais pontos de dado estilo HUD: specs da Imagery, coluna "are" + terminologia da Voice, valores da cadeia de tokens em Tokens.
    - [x] Todos os três respeitam `prefers-reduced-motion` (renderizam o estado final instantaneamente).
    - [x] **Bug arquitetural encontrado e corrigido:** primeira versão do `ScrambleHover` usava `forwardRef`/`useImperativeHandle` com o ref vindo do componente pai — mas `SectionShell` renderiza `body` DUAS vezes (wrapper desktop `hidden lg:block` + wrapper mobile `lg:hidden`, ambos montados ao mesmo tempo no DOM), então as duas instâncias brigavam pelo mesmo `ref.current` e só a última montada "vencia" — o hover funcionava (CSS mudava), mas o texto nunca embaralhava porque a chamada ia pra instância errada, invisível. Falhava em silêncio, sem erro. Corrigido reescrevendo o componente pra ser autocontido: encontra seu próprio ancestral interativo (`element.closest("a, button")`) e escuta `mouseenter`/`focus` diretamente nele, sem depender de ref externo — funciona corretamente não importa quantas cópias existam. **Esta é a terceira vez que o padrão de renderização dupla do `SectionShell` causa um bug silencioso desse tipo** (as duas anteriores envolveram o escopo do `--u`) — ao adicionar qualquer novo componente interativo/stateful dentro de `heading`/`body`, checar primeiro se ele depende de ref único.
  - **E. Imagens em alta qualidade** ✅ (parcial, limite real documentado)
    - [x] Achado real sem trade-off: a textura topográfica de fundo (`topo-orange.webp`, 640×669, esticada pra tela inteira em todas as 9 seções de conteúdo + capa) tinha equivalente vetorial nativo (`pattern-tile.svg`, já usado em Patterns) — trocado nos dois lugares (`SectionShell` e `CoverSection`), ganho de nitidez real e sem perda, arquivos raster órfãos deletados. Capa usa `filter: brightness(0) invert(1)` pra forçar branco a partir do SVG nativo Fire Orange.
    - [x] *(ajuste 2026-08-03)* A nitidez do SVG deixou a textura mais visível que o pretendido — opacidade reduzida em 25% relativo em ambos os pontos: seções de conteúdo `0.09` → `0.0675`, capa `0.06` → `0.045`.
    - [x] `cover-photo.webp` (1024×568) e `brand/portrait.webp` (1200×1552) não têm original em resolução maior em lugar nenhum do repositório — são fotos, não vetores, e não há como melhorá-las sem um novo arquivo-fonte do Lucas. **Pendência registrada, não resolvida.**
  - **F. Técnico** ✅
    - [x] **Lighthouse** — rodado contra build de produção real (`next build` + `serve`, nunca contra o dev server, que dá métrica falsa). Score final: **Performance 100 · Acessibilidade 100 · Boas Práticas 100 · SEO 100**. Achados corrigidos: `<title>` da página também tinha um em-dash; faltava `robots.txt` (`src/app/robots.ts` novo, precisa de `export const dynamic = "force-static"` pra funcionar em export estático — armadilha do Next 16 + Turbopack); botões de swatch da Color tinham `aria-label` que descartava o texto visível inteiro (violação de "Label in Name") — trocado por `<span className="sr-only">` que complementa em vez de substituir.
    - [x] **Acessibilidade manual** (além do Lighthouse): lightbox da Imagery não tinha focus trap nem devolvia o foco ao fechar — corrigido (foco vai pro botão fechar ao abrir, Tab fica preso nele, Esc devolve foco pro botão que abriu — os três comportamentos testados de verdade via CDP, não só por leitura de código). Menu mobile ganhou o mesmo tratamento de Esc + `role="dialog"`.
    - [x] **Reduced-motion** — testado com `Emulation.setEmulatedMedia` via CDP (não só leitura de código): Cover, `SectionShell`, `CountUp` e `DecodeText` mostram o estado final instantaneamente, sem esperar animação. Achado e corrigido de passagem: `DecodeText` chamava `setState` de forma síncrona dentro do efeito pro caminho reduced-motion (erro de lint `react-hooks/set-state-in-effect`) — corrigido renderizando `text` direto nesse caminho, sem precisar de state.
    - [x] **Bug de lint sério achado e corrigido:** `SectionShell.tsx` definia dois componentes (`Heading`/`Body`) **dentro** do corpo do componente principal — erro `react-hooks/static-components` (identidade do componente reseta a cada render). Movidos pro escopo do módulo, recebendo `heading`/`body` como props em vez de closure. Esse era o "1 Issue" que aparecia no overlay de dev do Next a cada seção.
    - [x] **Cross-browser** — sem Firefox/Safari real disponível neste ambiente Windows (só Chrome headless via CDP). Auditoria de código: o site usa `backdrop-filter`, `color-mix()` e `clip-path` — todos suportados em Chrome/Firefox/Safari desde 2023, com degradação graciosa (não quebra) em navegador mais antigo. **Limitação registrada, não testado visualmente fora do Chrome.**
    - [x] **Passada mobile completa** — as 13 páginas (todas as seções + sub-páginas) capturadas em 390×844 depois de todas as mudanças acima; nenhuma quebra, overflow ou texto cortado.
- ✅ **Fase 11.5 — Revisão de textos** *(2026-08-03)*: seção final "11 — Close" adicionada (frase de marca + links GitHub/Instagram/Hope Channel, layout próprio como a Capa, índice fixo também some nela). Todo heading de seção ganhou um kicker de duas linhas: nome literal da seção em Heading 3 (28px, fire, typewriter) acima da frase de impacto, agora Label Large (16px, branco) — 14 chamadas de `SectionShell` atualizadas. Copy revisada com `humanizer` (2 em-dashes reais achados nos links da seção Close, corrigidos). Nova varredura de viúvas via `Range.getClientRects()` em 1440 **e** 390px (não só desktop) achou 9 casos reais (Color, Voice terminology, System×2, Logos misuse, Typography, Tokens×2) — todos corrigidos por reescrita; a de Logos misuse precisou de várias rodadas porque o texto que resolvia num viewport quebrava no outro.
- ✅ **Fase 12 — Deploy produção** *(2026-08-03)*: live em **https://tte-brand-system.vercel.app/** (Vercel, projeto conectado ao repo `lmtts/tte-brand-system`, Root Directory `04-brand-book/site`). Pego de primeira: Root Directory errado (`04-brand-book` sem o `/site`) — corrigido, redeploy ok. Todo push no `main` agora dispara deploy automático. Domínio próprio ainda a decidir.
...
| 2026-08-03 | Rodada 3 de motion: `ScrambleHover` estendido pra Cover (v1.0/2026/Brand Guidelines, com `revealOnMount`), hex/nomes da Color, "Aa" da Typography, e portado pro `Button` real do `03-dev-system/tte-ui` (não só o site do brand book). Logos ganhou a 2ª página `logos-misuse` (as 6 regras oficiais da spec do Figma, cada uma demonstrada ao vivo no asset real). Bug real corrigido: `bg-biome-${key}` no Biome Badge do System usava classe Tailwind construída em runtime — Tailwind v4 tree-shaka do `:root` qualquer `--color-*` de `@theme` cujo utility não apareça como classe LITERAL no código, então só "desert" sobrevivia (usado literalmente em outro lugar); trocado por hex direto no componente. Pendente: grid 2×2 da Imagery e 2º card de people group no System aguardam imagens novas do Lucas. |
