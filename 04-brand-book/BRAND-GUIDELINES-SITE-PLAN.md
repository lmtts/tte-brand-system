...
- 🟡 **Fase 11 — Polish global** *(iniciada 2026-07-31, maior parte concluída no mesmo dia)*: escopo consolidado (planejamento original + notas do Lucas coletadas ao longo da montagem).
  - **A. Conteúdo & copy** ✅
  ...
  - **C. Navegação por seta** ✅
    - [x] `ArrowNav.tsx` novo: seta cima/baixo no teclado + botões fixos prev/next (canto inferior direito, acima do rodapé) — os dois juntos. Novo hook `useActivePageId` (granularidade de página, não só seção) pra saber exatamente onde parar. Navegação suprimida quando há um `[role="dialog"]` aberto (lightbox), pra não navegar a página inteira por baixo dele.
  - **D. Motion de intel/HUD — ainda BLOQUEADO:** Lucas ainda não mandou a referência do efeito específico.
