...
- ✅ **Fase 9 — 09 System** *(2026-07-31)*: sem frame; proposta em código a partir da seção 13 do `DESIGN.md` ("Design System — components": base shadcn/ui, escala de tamanho de controles, a regra bandeira dos botões, os organisms exclusivos da TTE). Terceira seção multi-página (`pageIds:["system","system-organisms"]`). Página 1 `system` = a **regra bandeira dos botões** (`intent` fixa a tipografia por papel — Mona Sans mobiliza, Space Mono opera — `variant` fixa o tratamento visual) demonstrada com 4 botões **reais** (recriados visualmente com as regras de altura/radius/tipografia do token, não importados do pacote `tte-ui`): Mobilize/Primary "PRAY" (Fire fill), Mobilize/Secondary "GIVE" (outline), Operate/Primary "FILTER", Operate/Secondary "EXPORT" — todos Mona Sans em cima, Space Mono embaixo, tornando o eixo visualmente óbvio. Lista compacta da escala de controle (36/44/56px) abaixo do texto. Página 2 `system-organisms` = os 5 componentes exclusivos da marca (HUD Panel, People Group Card, Mission Stat, Biome Badge, Topographic Background) como cards descritivos, mesmo padrão de Patterns. Único bug pego antes do screenshot: `type ButtonSpec` tinha uma sintaxe de union inválida (`"Mobilize" || string`, não é TypeScript válido) — corrigido pra `"Mobilize" | "Operate"` antes mesmo de rodar o typecheck.
